...
def executar_grid_search(X_train_tfidf, y_train):
    print("Executant Grid Search per Naive Bayes...")
    param_grid = {
...
def main():
    ...
    plt.title('Matriu de Confusió - Naive Bayes (Grid Search)')
    plt.xlabel('Prediccions')
    plt.ylabel('Valors Reals')
    plt.savefig(os.path.join(output_dir, 'matriu_confusio_naive_bayes_grid_search.png'))
    plt.show()
